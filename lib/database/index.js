...
Database.prototype.find = function find(collectionName, criteria, cb) {
  ...
  if(criteria.where && Utils.present(criteria.where[primary])) {
    ...
    // Otherwise, check to see if we have an index for this particular criteria
    var indices = self.schema._indices[name];
    for(var x = 0; x < indices.length; x++) {
      var index = indices[x];
      if(criteria.where && Utils.present(criteria.where[index.name])) {
        indexKey = self.schema.recordKey(name, index.name, criteria.where[index.name]);
      }
    };
...
Database.prototype.create = function create(collectionName, data, cb) {
  ...
  async.auto({
    ...
    // Add indexed values to indexed set
    addToIndexedSet: ['saveKeyData', function(next, results) {

      var primaryKey = self.schema.recordKey(name, primary, data[primary]);
      var addIndexValue = function(index, callback) {
        if(primary === index.name) {
           return callback(null);
        }
        if(data[index.name]) {
          var noSqlKey = self.schema.recordKey(name, index.name, data[index.name]);
          self.connection.sadd(noSqlKey, primaryKey, callback);
        } else {
          callback(null);
        } 
      };
      ...
      var indices = self.schema._indices[name];
      async.each(indices, addIndexValue, function() {
        next(null, results);
      });
...
Database.prototype.destroy = function destroy(collectionName, criteria, cb) {
  ...
  async.auto({
    ...
    // remove indexed values from indexed set
    destroyIndexedSets: ['findRecords', function(next, results) {
      var data = results.findRecords;
      var removeIndexValue = function(index, callback) {
        if(primary === index.name) {
           return callback(null);
        }
        async.each(data, function(item, callback) {
          if(item[index.name]) {
            var primaryKey = self.schema.recordKey(name, primary, item[primary]);
            var noSqlKey = self.schema.recordKey(name, index.name, item[index.name]);
            return self.connection.srem(noSqlKey, primaryKey, callback); 
          }
          callback(null);
        }, callback);
      };
      ...
      var indices = self.schema._indices[name];
      async.each(indices, removeIndexValue, function() {
        next(null, results);
      });
...
Database.prototype._uniqueConstraint = function _uniqueConstraint(collectionName, data, cb) {
  ...
  var primary = this.schema._primary[collectionName];
  ...
  indices.forEach(function(idx) {
    ...
    // If its not a unique index, also ignore it
